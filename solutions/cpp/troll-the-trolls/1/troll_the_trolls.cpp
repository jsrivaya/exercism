...
bool has_priority(AccountStatus player1_status, AccountStatus player2_status) {
    if (AccountStatus::mod == player1_status && AccountStatus::mod != player2_status) {
        return true;
    }
    if ((AccountStatus::user == player1_status && AccountStatus::guest == player2_status) ||
        (AccountStatus::user == player1_status && AccountStatus::troll == player2_status)) {
        return true;
    }
    if (AccountStatus::guest == player1_status && AccountStatus::troll == player2_status) {
        return true;
    }
    if (AccountStatus::troll == player2_status && AccountStatus::troll != player1_status) {
        return true;
    }
    return false;
}
